...
class Utilisateur:
    def __init__(self, id_utilisateur, prenom, nom, identifiant, courriel,
                 photo, actif):
...
# Classe modélisant la base de données utilisée pour l'application
class Database:
    ...
    def ajouter_utilisateur(self, prenom, nom, identifiant, courriel, hach,
                            salt, photo):
        photo_binaire = sqlite3.Binary(photo.read())
        connection = self.get_connection()
        connection.execute(
            "INSERT INTO utilisateur (prenom, nom, identifiant, courriel, "
            "hach, salt, photo_profil, actif) VALUES (?, ?, ?, ?, ?, ?, ?, 1)",
            (prenom, nom, identifiant, courriel, hach, salt,
             photo_binaire))
        connection.commit()

    def ajouter_article(self, identifiant, titre, auteur_id, date, contenu,
                        est_html):
        connection = self.get_connection()
        connection.execute("INSERT INTO article (identifiant, titre, "
                           "auteur_id, date_publication, contenu, est_html)"
                           "VALUES (?, ?, ?, ?, ?, ?)", (identifiant,
                                                         titre, auteur_id,
                                                         date, contenu,
                                                         est_html))
    ...
    def get_article(self, identifiant):
        ...
        if resultat:
            date = (datetime.strptime(resultat[3], "%Y-%m-%d %H:%M:%S.%f")
                    .strftime("%Y-%m-%d - %H:%M"))
            return Article(identifiant=resultat[0], titre=resultat[1],
                           auteur=resultat[2], date=date, contenu=resultat[4],
                           est_html=resultat[5])
        else:
            return None
    ...
    def get_photo_auteur(self, auteur_id):
        cursor = self.get_connection().cursor()
        cursor.execute("SELECT photo_profil FROM utilisateur "
                       "WHERE id = ?", (auteur_id,))
        resultat = cursor.fetchone()
        return resultat[0]
    ...
    def get_nom_auteur(self, auteur_id):
        cursor = self.get_connection().cursor()
        cursor.execute("SELECT prenom || ' ' || nom AS nom_complet FROM "
                       "utilisateur WHERE id = ?",
                       (auteur_id,))
    ...
    def get_all_articles(self):
        ...
        if len(articles) != 0:
            ...
            for article in articles:
                date_formatee = (datetime.strptime(article[3], "%Y-%m-%d "
                                                               "%H:%M:%S.%f")
                                 .strftime("%Y-%m-%d - %H:%M"))
                liste_articles.append(
                    Article(identifiant=article[0], titre=article[1],
                            auteur=article[2], date=date_formatee,
    ...
    def get_articles_like(self, recherche):
        cursor = self.get_connection().cursor()
        cursor.execute(
            "SELECT * FROM article WHERE titre LIKE ? OR contenu LIKE ?",
            ('%' + recherche + '%', '%' + recherche + '%'))
        articles = cursor.fetchall()
        if len(articles) != 0:
            liste_articles = []
            for article in articles:
                date_formatee = (datetime.strptime(article[3],
                                                   "%Y-%m-%d %H:%M:%S.%f")
                                 .strftime("%Y-%m-%d - %H:%M"))
                liste_articles.append(
                    Article(identifiant=article[0], titre=article[1],
    ...
    def get_all_utilisateurs(self):
        ...
        for utilisateur in utilisateurs:
            liste_utilisateurs.append(
                Utilisateur(id_utilisateur=utilisateur[0],
    ...
    def authentifier(self, identifiant, mot_de_passe):
        cursor = self.get_connection().cursor()
        cursor.execute("SELECT * FROM utilisateur WHERE identifiant == ?",
                       (identifiant,))
    ...
    def get_session(self, id_session):
        cursor = self.get_connection().cursor()
        cursor.execute("SELECT * FROM session WHERE id_session = ?",
                       (id_session,))
    ...
    def get_utilisateur(self, id_utilisateur):
        cursor = self.get_connection().cursor()
        cursor.execute("SELECT * FROM utilisateur WHERE id = ?",
                       (id_utilisateur,))
        utilisateur = cursor.fetchone()
        if utilisateur:
            return Utilisateur(id_utilisateur=utilisateur[0],
                               prenom=utilisateur[1], nom=utilisateur[2],
    ...
    def desactiver_utilisateur(self, id_utilisateur):
        connection = self.get_connection()
        connection.execute("UPDATE utilisateur SET actif = 0 WHERE id = ?",
                           (id_utilisateur,))
        connection.commit()
    ...
    def modifier_article(self, identifiant, titre, contenu, est_html):
        connection = self.get_connection()
        connection.execute(
            "UPDATE article SET titre = ?, contenu = ?, est_html = ? "
            "WHERE identifiant = ?", (titre, contenu, est_html,
                                      identifiant))
        connection.commit()

    def modifier_utilisateur(self, prenom, nom, identifiant, courriel, photo,
                             statut, id_utilisateur):
        connection = self.get_connection()
        if photo is not None:
            connection.execute(
                "UPDATE utilisateur SET prenom = ?, nom = ?, "
                "identifiant = ?, courriel = ?, photo_profil = ?, actif = ? "
                "WHERE id = ?", (prenom, nom, identifiant, courriel,
                                 sqlite3.Binary(photo.read()), statut,
                                 id_utilisateur))
        else:
            connection.execute(
                "UPDATE utilisateur SET prenom = ?, nom = ?, "
                "identifiant = ?, courriel = ?, actif = ?"" WHERE id = ?",
                (prenom, nom, identifiant, courriel, statut,
                 id_utilisateur))
        connection.commit()
